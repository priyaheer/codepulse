...
router.get('/:id/issues', requireAuth, async (req, res, next) => {
  try {
    const { default: { listIssues } } = await import('../controllers/issueController.js');
...
router.post('/:id/ask-ai', requireAuth, async (req, res, next) => {
  try {
    const { default: { askProjectAI } } = await import('../controllers/aiController.js');
    return askProjectAI(req, res, next);
  } catch (err) {
    next(err);
  }
});
