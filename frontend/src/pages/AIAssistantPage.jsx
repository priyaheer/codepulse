...
export function AIAssistantPage() {
  ...
  async function ask(value = question) {
    ...
    if (!hasApiSession() || !projectId) {
      setError('Connect a project and sign in before asking repository-grounded AI questions.');
      return;
    }
    ...
    setError('');
    try {
      setAnswer(await api.askProjectAI(projectId, value));
    } catch (err) {
        ...
        <Card>
          ...
          <CardBody className="space-y-4">
            ...
            <div className="rounded-md border border-border bg-background p-3 text-[13px] text-text-primary">
              ...
              <p>{answer ? `Confidence: ${Math.round(answer.confidence * 100)}%. Answers are limited to repository evidence sent for this question.` : 'No repository context has been sent yet.'}</p>
            </div>
            <div className="mt-4 flex items-center gap-2">
              ...
              <Button variant="primary" size="sm" icon={<Search size={13} />} onClick={() => ask()} disabled={loading}>{loading ? 'Asking...' : 'Ask'}</Button>
            </div>
          </CardBody>
        </Card>
      </div>
